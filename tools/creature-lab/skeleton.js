...
// -- Clean creature render (fungal shaman approach) -------------------------
// Bold shapes, clean fills, no noise. Let the ASCII renderer add texture.
// ~40 lines of drawing vs 250+ in the detailed version.
export function drawCleanCreature(ctx, skeleton, w, h, helpers) {
  const { bodyGradHueShift: _grad, fillCircle: _fc, fillEllipse: _fe } = helpers;
  ...
  for (const limb of skeleton.limbs) {
    if (limb.side === 'far') {
      drawLimbChain(ctx, limb.segments, w, h, 'rgb(24,16,12)');
    }
  }

  // 3. Body — ONE gradient, clean
  const bodyFill = _grad(ctx, w * belly.x, h * belly.y, w * 0.30,
    90, 68, 50,   // mid
    30, 22, 16,   // shadow
    130, 105, 80  // highlight
  );
  drawBodyOutline(ctx, skeleton.spine, w, h, bodyFill);
  ...
  for (const limb of skeleton.limbs) {
    if (limb.side === 'near') {
      drawLimbChain(ctx, limb.segments, w, h, 'rgb(42,30,22)');
    }
  }

  // 5. Paw pads — simple ellipses
  for (const limb of skeleton.limbs) {
    const paw = limb.segments[limb.segments.length - 1];
    ctx.fillStyle = limb.side === 'near' ? 'rgb(60,40,35)' : 'rgb(35,24,18)';
    _fe(ctx, w * paw.x, h * paw.y, w * (paw.radius + 0.005), h * 0.012);
  }

  // 6. Head — simple dark shape over spine head area
  const hx = head.x, hy = head.y, hr = head.radius;
  const sx = snout.x, sy = snout.y;
  ctx.fillStyle = _grad(ctx, w * hx, h * hy, w * hr * 2,
  ...
  // 7. Snout — small dark nub
  ctx.fillStyle = 'rgb(55,35,30)';
  ctx.beginPath();
  ctx.arc(w * (sx - 0.03), h * (sy + 0.01), w * 0.022, 0, Math.PI * 2);
  ctx.fill();

  // 8. Nose — bright wet dot
  ctx.fillStyle = 'rgb(150,80,75)';
  _fc(ctx, w * (sx - 0.045), h * (sy + 0.005), w * 0.008);

  // 9. Mouth slit — just a dark line
  ctx.strokeStyle = 'rgb(10,5,5)';
  ctx.lineWidth = w * 0.004;
  ctx.beginPath();
  ...
  const eyeX = hx - 0.01, eyeY = hy - 0.01;
  // Near eye — bright
  ctx.fillStyle = 'rgb(200,50,30)';
  _fc(ctx, w * eyeX, h * eyeY, w * 0.012);
  ctx.fillStyle = 'rgb(255,120,40)';
  _fc(ctx, w * eyeX, h * eyeY, w * 0.007);
  ctx.fillStyle = '#000';
  _fc(ctx, w * (eyeX + 0.001), h * eyeY, w * 0.003);
  // Far eye — dimmer
  ctx.fillStyle = 'rgb(160,40,25)';
  _fc(ctx, w * (sx + 0.005), h * (sy - 0.05), w * 0.008);

  // 12. Ears — small round nubs on top of skull
  ctx.fillStyle = 'rgb(50,36,28)';
  _fe(ctx, w * (hx + 0.06), h * (hy - hr * 0.9), w * 0.020, h * 0.030);
  ctx.fillStyle = 'rgb(38,26,20)';
  _fe(ctx, w * (hx - 0.01), h * (hy - hr * 0.8), w * 0.016, h * 0.024);
  // Pink inner
