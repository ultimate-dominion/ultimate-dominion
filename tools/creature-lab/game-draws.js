...
function drawDireRatRedux(ctx, w, h) {
...
function drawKoboldRedux(ctx, w, h) {
...
function drawGoblinRedux(ctx, w, h) {
...
function drawCavernBruteRedux(ctx, w, h) {
...
function drawCrystalElementalRedux(ctx, w, h) {
...
function drawIronhideTrollRedux(ctx, w, h) {
...
function drawPhaseSpiderRedux(ctx, w, h) {
  const far = 'rgb(92,74,148)';
  const near = 'rgb(148,128,214)';
  const farLegs = [[0.44,0.40,0.58,0.22,0.72,0.08],[0.50,0.42,0.64,0.30,0.78,0.18],[0.52,0.48,0.64,0.58,0.76,0.74],[0.48,0.52,0.58,0.68,0.68,0.86]];
  ...
  ctx.fill();
  for (const [x,y,r] of [[0.14,0.36,0.008],[0.17,0.34,0.010],[0.20,0.36,0.008],[0.15,0.39,0.007],[0.18,0.39,0.007],[0.21,0.39,0.006]]) {
    ctx.fillStyle = 'rgb(170,255,168)';
    fillCircle(ctx, w * x, h * y, w * r);
  }
  const nearLegs = [[0.28,0.48,0.16,0.28,0.06,0.12],[0.32,0.50,0.18,0.40,0.08,0.30],[0.40,0.56,0.28,0.70,0.18,0.84],[0.46,0.58,0.42,0.74,0.38,0.92]];
  for (const [x0,y0,cx,cy,x1,y1] of nearLegs) drawLimb(ctx, w*x0,h*y0,w*cx,h*cy,w*x1,h*y1,w*0.015,near);
}

function drawBonecasterRedux(ctx, w, h) {
...
function drawRockGolemRedux(ctx, w, h) {
...
function drawPaleStalkerRedux(ctx, w, h) {
...
function drawDuskDrakeRedux(ctx, w, h) {
...
function drawBasiliskRedux(ctx, w, h) {
  ...
  for (const [x, hgt] of [[0.36,0.08],[0.42,0.11],[0.49,0.12],[0.56,0.11],[0.62,0.09]]) {
...
export { MONSTER_TEMPLATES_REDUX };
