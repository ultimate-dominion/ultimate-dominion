...
function makeSyncHandle(overrides?: Partial<SyncHandle>): SyncHandle {
  return {
    ...
    tableNameMap: new Map([
      ...
      ['AdventureEscrow', 'ud__adventure_escrow'],
    ]),
    ...overrides,
  };
}
...
function makeRow(
  ...
  gold: { external: number; escrow: number },
  ...
  return {
    ...
    external_gold: gold.external,
    escrow_gold: gold.escrow,
    total_gold: gold.external + gold.escrow,
...
describe('GET /leaderboard/nearby', () => {
  ...
  it('returns nearby players for middle rank character', async () => {
    const charId = '0x' + 'aa'.repeat(32);
    const syncHandle = makeSyncHandle();
    const app = createApp(syncHandle);

    const rows = [
      makeRow('0x' + 'bb'.repeat(32), 'Player1', 8, { agi: 20, str: 20, int: 20 }, { external: 100, escrow: 50 }, { statsRank: 1, goldRank: 1 }, 5),
      makeRow('0x' + 'cc'.repeat(32), 'Player2', 7, { agi: 15, str: 15, int: 15 }, { external: 80, escrow: 30 }, { statsRank: 2, goldRank: 2 }, 5),
      makeRow(charId, 'Self', 6, { agi: 10, str: 10, int: 10 }, { external: 50, escrow: 20 }, { statsRank: 3, goldRank: 3 }, 5),
      makeRow('0x' + 'dd'.repeat(32), 'Player4', 5, { agi: 8, str: 8, int: 8 }, { external: 30, escrow: 10 }, { statsRank: 4, goldRank: 4 }, 5),
      makeRow('0x' + 'ee'.repeat(32), 'Player5', 4, { agi: 5, str: 5, int: 5 }, { external: 10, escrow: 5 }, { statsRank: 5, goldRank: 5 }, 5),
    ];
  ...
  it('returns nearby players for top rank character', async () => {
    ...
    const rows = [
      makeRow(charId, 'TopPlayer', 10, { agi: 30, str: 30, int: 30 }, { external: 200, escrow: 100 }, { statsRank: 1, goldRank: 1 }, 10),
      makeRow('0x' + 'bb'.repeat(32), 'Player2', 8, { agi: 20, str: 20, int: 20 }, { external: 100, escrow: 50 }, { statsRank: 2, goldRank: 2 }, 10),
      makeRow('0x' + 'cc'.repeat(32), 'Player3', 7, { agi: 15, str: 15, int: 15 }, { external: 80, escrow: 30 }, { statsRank: 3, goldRank: 3 }, 10),
    ];
  ...
  it('returns nearby players for bottom rank character', async () => {
    ...
    const rows = [
      makeRow('0x' + 'bb'.repeat(32), 'Player8', 3, { agi: 5, str: 5, int: 5 }, { external: 20, escrow: 10 }, { statsRank: 8, goldRank: 8 }, 10),
      makeRow('0x' + 'cc'.repeat(32), 'Player9', 2, { agi: 3, str: 3, int: 3 }, { external: 10, escrow: 5 }, { statsRank: 9, goldRank: 9 }, 10),
      makeRow(charId, 'BottomPlayer', 1, { agi: 1, str: 1, int: 1 }, { external: 1, escrow: 0 }, { statsRank: 10, goldRank: 10 }, 10),
    ];
  ...
  it('returns 503 when tables not yet synced', async () => {
    const syncHandle = makeSyncHandle({
      tableNameMap: new Map(), // empty — no tables synced
    });
    const app = createApp(syncHandle);
  ...
  it('works with gold ranking', async () => {
    ...
    const rows = [
      makeRow(charId, 'Self', 5, { agi: 5, str: 5, int: 5 }, { external: 100, escrow: 50 }, { statsRank: 5, goldRank: 2 }, 10),
      makeRow('0x' + 'bb'.repeat(32), 'RichPlayer', 3, { agi: 3, str: 3, int: 3 }, { external: 200, escrow: 100 }, { statsRank: 8, goldRank: 1 }, 10),
      makeRow('0x' + 'cc'.repeat(32), 'Player3', 4, { agi: 4, str: 4, int: 4 }, { external: 50, escrow: 30 }, { statsRank: 6, goldRank: 3 }, 10),
    ];
  ...
  it('handles missing GoldBalances/AdventureEscrow tables gracefully', async () => {
    const charId = '0x' + 'aa'.repeat(32);
    const syncHandle = makeSyncHandle({
      tableNameMap: new Map([
        ['Characters', 'ud__characters'],
        ['Stats', 'ud__stats'],
        // No GoldBalances or AdventureEscrow
      ]),
    });
    const app = createApp(syncHandle);

    const rows = [
      makeRow(charId, 'Self', 5, { agi: 10, str: 10, int: 10 }, { external: 0, escrow: 0 }, { statsRank: 1, goldRank: 1 }, 1),
    ];
