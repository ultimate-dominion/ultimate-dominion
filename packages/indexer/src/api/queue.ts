...
export function createQueueRouter(syncHandle: SyncHandle, broadcaster: Broadcaster): Router {
  ...
  router.post('/join', async (req, res) => {
    try {
      ...
      // Determine priority
      let priority: 'founder' | 'invited' | 'normal' = 'normal';
 ...
 */
async function checkFounderBadge(syncHandle: SyncHandle, wallet: string): Promise<boolean> {
  try {
    const ownersTable = syncHandle.tableNameMap.get('BadgesOwners');
    if (!ownersTable) return false;
    ...
    const rows = await sql.unsafe(
      `SELECT * FROM "${mudSchema}"."${ownersTable}" WHERE "owner" = $1`,
      [wallet.toLowerCase()]
    );
    ...
    for (const row of rows) {
      ...
      if (tokenId !== undefined && Number(tokenId) === 50) return true;
    }
 ...
 */
async function getCurrentPlayerInfo(syncHandle: SyncHandle): Promise<{
  ...
  try {
    ...
    if (configTable) {
      const rows = await sql.unsafe(
        `SELECT "max_players" FROM "${mudSchema}"."${configTable}" LIMIT 1`
      );
