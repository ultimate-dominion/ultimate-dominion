  ...
  router.get('/nearby', async (req, res) => {
    try {
      ...
      const nearby = rows.map((r: any) => {
        ...
        // Serialize ALL values — MUD stores strings as bytes, so r.name
        // is a Buffer. serializeValue converts Buffers to hex strings.
        const rawName = serializeValue(r.name);
        // name is stored as bytes — decode hex to UTF-8 string
        const name = typeof rawName === 'string' && rawName.startsWith('0x')
...
/** Decode a 0x-prefixed hex string to UTF-8, stripping trailing null bytes. */
