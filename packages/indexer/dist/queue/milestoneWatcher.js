...
export async function generateInviteCode(wallet, milestone) {
...
async function hasCodeForMilestone(wallet, milestone) {
 ...
 */
export function startMilestoneWatcher(syncHandle, broadcaster) {
 ...
 */
async function checkAndActivateReferral(inviteeWallet) {
    try {
    ...
    `;
        if (activations.length === 0)
            return;
        const inviteCode = activations[0].invite_code;
        // Find the referrer
    ...
    `;
        if (codeRows.length === 0)
            return;
        const referrerWallet = codeRows[0].creator_wallet;
...
//# sourceMappingURL=milestoneWatcher.js.map