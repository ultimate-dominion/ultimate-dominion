...
export function createBattleRouter(syncHandle) {
    const router = Router();
    /**
     * GET /:characterId
    ...
    router.get('/:characterId', async (req, res) => {
        try {
            ...
            if (!encounterEntityTable)
                return res.status(503).json({ error: 'Tables not yet synced' });
            // Find character's encounter entity
            const encounterEntityRows = await sql.unsafe(`SELECT * FROM "${mudSchema}"."${encounterEntityTable}" WHERE "encounter_entity_id" = $1`, [hexToBuffer(characterId)]);
            if (encounterEntityRows.length === 0) {
                return res.json({ encounter: null, outcomes: [], combatOutcome: null });
            }
            const encounterEntity = serializeRow(encounterEntityRows[0]);
            const encounterId = encounterEntity.encounterId;
            ...
            const [encounterRows, outcomeRows, combatOutcomeRows] = await Promise.all([
                encounterTable
                    ? sql.unsafe(`SELECT * FROM "${mudSchema}"."${encounterTable}" WHERE "encounter_id" = $1`, [hexToBuffer(encounterId)])
                    : [],
                outcomeTable
                    ? sql.unsafe(`SELECT * FROM "${mudSchema}"."${outcomeTable}" WHERE "encounter_id" = $1 ORDER BY "current_turn", "attack_number"`, [hexToBuffer(encounterId)])
                    : [],
                combatOutcomeTable
                    ? sql.unsafe(`SELECT * FROM "${mudSchema}"."${combatOutcomeTable}" WHERE "encounter_id" = $1`, [hexToBuffer(encounterId)])
                    : [],
            ]);
            res.json({
                encounter: encounterRows[0] ? serializeRow(encounterRows[0]) : null,
                encounterEntity,
                outcomes: outcomeRows.map((r) => serializeRow(r)),
                combatOutcome: combatOutcomeRows[0] ? serializeRow(combatOutcomeRows[0]) : null,
