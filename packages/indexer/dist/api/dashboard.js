...
function formatDetail(name, svc) {
    const d = svc.details || {};
    const lines = [];
    switch (name) {
        case 'base-node': {
            if (d.blockLag !== undefined)
                lines.push(`Lag: ${d.blockLag} blocks`);
            const rm = d.rethMetrics;
            if (rm) {
                if (rm.memoryMb !== null && rm.memoryMb !== undefined)
                    lines.push(`Mem: ${(rm.memoryMb / 1000).toFixed(1)}G`);
                if (rm.peers !== null && rm.peers !== undefined)
                    lines.push(`Peers: ${rm.peers}`);
                if (rm.activeConnections !== null && rm.activeConnections !== undefined)
                    lines.push(`Conns: ${rm.activeConnections}`);
            }
            break;
        }
        case 'relayer':
            if (d.poolSize !== undefined)
                lines.push(`Pool: ${d.poolSize}`);
            if (d.totalInflight !== undefined)
                lines.push(`Inflight: ${d.totalInflight}`);
            if (d.lowBalanceWallets)
                lines.push(`Low bal: ${d.lowBalanceWallets}`);
            break;
        case 'indexer':
            if (d.lag !== undefined)
                lines.push(`Lag: ${d.lag}`);
            if (d.wsClients !== undefined)
                lines.push(`WS: ${d.wsClients}`);
            break;
        default:
            break;
    }
...
function sparkline(services, name) {
...
function renderServiceCard(name, svc, allServices) {
...
function renderDashboard(status) {
    ...
    const serviceOrder = ['base-node', 'relayer', 'indexer', 'client-prod', 'client-beta', 'api'];
