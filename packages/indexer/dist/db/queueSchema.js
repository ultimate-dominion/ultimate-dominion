...
export async function setPlayerEmail(wallet, email) {
    await sql `
...
export async function getPlayerEmail(wallet) {
...
export async function shouldNotifyAndMark(wallet) {
...
export async function getQueuePosition(wallet) {
  ...
  `;
    return {
        ...
        status: entry.status,
        readyUntil: entry.ready_until ? new Date(entry.ready_until) : null,
    };
}
...
export async function joinQueue(wallet, priority, inviteCodeUsed) {
    const w = wallet.toLowerCase();
...
export async function leaveQueue(wallet) {
    const result = await sql `
...
export async function acknowledgeSlot(wallet) {
    const result = await sql `
    UPDATE queue.queue_entries
    SET status = 'ready'
...
export async function markSpawned(wallet) {
    const result = await sql `
    UPDATE queue.queue_entries
    SET status = 'spawned'
    WHERE wallet = ${wallet.toLowerCase()} AND status = 'ready'
 ...
 */
export async function advanceQueue(slotsAvailable) {
    if (slotsAvailable <= 0)
        return [];
...
//# sourceMappingURL=queueSchema.js.map