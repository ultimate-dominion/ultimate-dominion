...
async function waitForEmbeddedIdentityToken(
  ...
  const configuredTimeoutMs = Number(
    import.meta.env.VITE_EMERGENCY_FUNDING_IDENTITY_WAIT_MS,
  );
...
async function requestEmergencyFunding(
  ...
  if (isEmbeddedFunding && !identityToken) {
    console.warn(
      '[GAS_RETRY] Embedded wallet funding skipped: identity token unavailable',
    );
    return false;
  }
...
function isInsufficientFundsError(error: unknown): boolean {
  if (error && typeof error === 'object' && 'walk' in error) {
    const found = (error as BaseError).walk(
...
const KNOWN_ERROR_SIGNATURES: Record<string, string> = {
  ...
  '0x261fa6d6':
    'Your character is bound by an ancient seal and cannot be changed right now.',
  '0x82b42900':
    'The cave wards block your path. You lack the authority for this.',
  ...
  '0xa17bea2c':
    'You need a moment to catch your breath before adventuring again.',
...
const classifyError = (
  error: unknown,
): { category: ErrorCategory; message: string; selector?: string } => {
  ...
  if (error && typeof error === 'object' && 'walk' in error) {
    ...
    if (revertError instanceof ContractFunctionRevertedError) {
      ...
      const requireMessage =
        errorName === 'Error' && args[0] ? (args[0] as string) : undefined;
      const reason =
        requireMessage ??
        knownMsg ??
        (errorName && errorName !== 'Error' ? errorName : undefined) ??
        (args[0] as string) ??
        `Unknown revert (${selector ?? 'no selector'})`;
      return {
        category: 'REVERT',
        message: reason,
  ...
  if (
    /timeout|network|econnrefused|econnreset|socket hang up|fetch failed|failed to fetch|rate limit|429|502|503|504/i.test(
      raw,
    )
  ) {
  ...
  return {
    category: 'UNKNOWN',
    message: 'Something stirs in the darkness. Try again.',
  };
};
...
export function createSystemCalls({
  ...
  const proxiedWrite = new Proxy(worldContract.write, {
    get(target, prop, receiver) {
      ...
      return async (...args: unknown[]) => {
        try {
          return await (original as (...a: unknown[]) => Promise<unknown>)(
            ...args,
          );
        } catch (error) {
          ...
          console.info(
            `[GAS_RETRY] Insufficient funds on ${String(prop)}, requesting emergency top-up`,
          );
          ...
          if (fundResult === 'has_funds') {
            ...
            console.info(
              '[GAS_RETRY] Relayer says balance is sufficient, retrying immediately',
            );
            return await (original as (...a: unknown[]) => Promise<unknown>)(
              ...args,
            );
  ...
  const isMonsterAlive = (monsterId: string): boolean => {
    const ee = getTableValue('EncounterEntity', monsterId) as
      | { died?: boolean }
      | undefined;
    const sp = getTableValue('Spawned', monsterId) as
      | { spawned?: boolean }
      | undefined;
  ...
  const evictGhostMonsters = (monsterIds: string[]) => {
    ...
    if (typeof store.applyBatch === 'function') {
      ...
      for (const update of batch) {
        store.setRow(update.table, update.keyBytes, update.data ?? {});
      }
    }

    // Protect evicted rows from being re-added by stale WS updates.
    markEvictedRows(
      batch.map(update => ({ table: update.table, keyBytes: update.keyBytes })),
    );

    if (uniqueMonsterIds.length === 1) {
      console.warn(
        `[ghost] Evicted ghost monster ${uniqueMonsterIds[0].slice(0, 10)}`,
      );
      return;
    }

    console.warn(
      `[ghost] Evicted ${uniqueMonsterIds.length} targeted ghost monsters`,
    );
  };
  ...
  const SPAWNED_TABLE_ID =
    '0x74625544000000000000000000000000537061776e6564000000000000000000' as const;
  const ENCOUNTER_ENTITY_TABLE_ID =
    '0x74625544000000000000000000000000456e636f756e746572456e7469747900' as const;
  ...
  const POSITION_TABLE_ID =
    '0x74625544000000000000000000000000506f736974696f6e5632000000000000' as const;
  ...
  const readStoreRecord = async (
    ...
    try {
      ...
      console.warn(
        `[store] Failed reading ${label} for ${entityId.slice(0, 10)}`,
        error,
      );
      return null;
    }
  ...
  const syncMonsterSnapshot = (
    ...
    if (snapshot.encounter) {
      ...
      if (
        (currentEncounter?.encounterId ?? zeroHash) !== encounterId ||
        Boolean(currentEncounter?.died) !== died
      ) {
  ...
  const validateTileMonsters = async (
    ...
    const results = await Promise.allSettled(
      monsterIds.map(async id => {
        ...
        const [spawnedRecord, encounterRecord, chainPosition] =
          await Promise.all([
            readStoreRecord(SPAWNED_TABLE_ID, id as `0x${string}`, 'Spawned'),
            readStoreRecord(
              ENCOUNTER_ENTITY_TABLE_ID,
              id as `0x${string}`,
              'EncounterEntity',
            ),
            expectedPosition
              ? syncMonsterPositionFromChain(id)
    ...
    for (const r of results) {
      ...
      const inEncounter = Boolean(
        encounter && encounter.encounterId !== zeroHash,
      );
      const positionIsCleared =
        chainPosition !== null &&
        chainPosition.x === 0 &&
        chainPosition.y === 0;
      const offTile = Boolean(
        expectedPosition &&
          chainPosition &&
          !positionIsCleared &&
          (chainPosition.x !== expectedPosition.x ||
            chainPosition.y !== expectedPosition.y),
      );
      const isGhost =
        spawned === false ||
        Boolean(encounter?.died) ||
        offTile ||
        positionIsCleared;
      ...
      if (offTile) {
        ...
        console.warn(
          `[validateTile] Evicted cleared-position monster ${id.slice(0, 10)}`,
        );
      } else if (isGhost) {
  ...
  const isGhostMonsterError = (e: unknown): boolean => {
    ...
    return (
      msg.includes(INVALID_COMBAT_ENTITY_SELECTOR) ||
  ...
  const validateCharacterOwnership = (
    ...
    if (
      expectedOwner &&
      character.owner.toLowerCase() !== expectedOwner.toLowerCase()
    ) {
  ...
  const buy = async (
    ...
    try {
      ...
      return {
        error:
          txResult.status === 'success' ? undefined : 'Failed to buy item.',
  ...
  const cancelOrder = async (orderHash: string): SystemCallReturn => {
    try {
      const tx = await wrappedWorldContract.write.UD__cancelOrder([
        orderHash as Hash,
      ]);
      const txResult = await waitForTransaction(tx);
      ...
      return {
        error:
          txResult.status === 'success' ? undefined : 'Failed to cancel order.',
        success: txResult.status === 'success',
      };
    } catch (e) {
      return {
  ...
  const createEncounter = async (
    ...
    try {
      ...
      console.info(
        `[createEncounter] DEBUG type=${encounterType} group1=[${group1.join(',')}] group2=[${group2.join(',')}]`,
      );

      const tx = await wrappedWorldContract.write.UD__createEncounter(
        [encounterType, group1 as `0x${string}`[], group2 as `0x${string}`[]],
        gasOverride,
      );
  ...
  const createOrder = async (order: NewOrder): SystemCallReturn => {
    try {
      ...
      return {
        error:
          txResult.status === 'success' ? undefined : 'Failed to create order.',
        success: txResult.status === 'success',
      };
    } catch (e) {
      return {
  ...
  const endTurn = async (
    ...
    if (
      !combatEncounter ||
      BigInt(combatEncounter.end as string | number) !== BigInt(0)
    ) {
      return { success: true };
    }
    ...
    try {
      ...
      const tx = await wrappedWorldContract.write.UD__endTurn(
        [encounterId as `0x${string}`, playerId as `0x${string}`, actions],
        {
          gas: BigInt('10000000'),
        },
      );
      ...
      if (receipt.status !== 'success') {
        ...
        return {
          error: 'Attack reverted on-chain (unknown reason)',
          success: false,
        };
      }
      return { success: true };
  ...
  const enterGame = async (
    ...
    const ownershipError = validateCharacterOwnership(
      characterEntity,
      'enterGame',
    );
    if (ownershipError) return ownershipError;
  ...
  const fulfillOrder = async (orderHash: string): SystemCallReturn => {
    try {
      ...
      if (txResult.status === 'reverted') {
        ...
        try {
          await worldContract.simulate.UD__fulfillOrder([orderHash as Hash], {
            account: diagAccount,
          });
        } catch (diagError) {
          return { success: false, error: getContractError(diagError) };
  ...
  const isInvalidMoveError = (e: unknown): boolean => {
    const msg = String(e).toLowerCase();
    return (
      msg.includes(INVALID_MOVE_SELECTOR) ||
      msg.includes('invalid move') ||
      msg.includes('invalidmove')
    );
  };
  ...
  const isNotSpawnedError = (e: unknown): boolean => {
    const msg = String(e).toLowerCase();
    return (
      msg.includes(NOT_SPAWNED_SELECTOR) ||
      msg.includes('not spawned') ||
      msg.includes('notspawned')
    );
  };
  ...
  const onMoveSuccess = () => {
    ...
    if (
      consecutiveSuccesses >= DECAY_AFTER_SUCCESSES &&
      currentMoveGapMs > BASE_MOVE_GAP_MS
    ) {
      currentMoveGapMs = BASE_MOVE_GAP_MS;
      console.info(
        `[move] Throttle decayed to ${BASE_MOVE_GAP_MS}ms after ${consecutiveSuccesses} successes`,
      );
    }
  };
  ...
  const onMoveRevert = () => {
    ...
    if (currentMoveGapMs < BACKPRESSURE_MOVE_GAP_MS) {
      ...
      console.warn(
        `[move] Throttle widened to ${BACKPRESSURE_MOVE_GAP_MS}ms after revert`,
      );
    }
  };
  ...
  const isTransientRpcError = (e: unknown): boolean => {
    const msg = String(e).toLowerCase();
    return /failed to fetch|timeout|network|econnrefused|econnreset|socket hang up|rate limit|429|502|503|504/.test(
      msg,
    );
  };

  const MOVE_STALE_STATE_WARNING =
    "You're moving too fast! Take a moment and try again.";
  ...
  const move = async (
    ...
    if (
      ...
      encounterState.encounterId !== '0x' + '0'.repeat(64)
    ) {
    ...
    const pos = (getTableValue('PositionV2', characterEntity) ??
      getTableValue('Position', characterEntity)) as
      | { x: number; y: number }
      | undefined;
    if (!pos) {
    ...
    try {
      ...
      let args: [`0x${string}`, number, number] = [
        characterEntity as `0x${string}`,
        x,
        y,
      ];
      ...
      const diagnoseMove = async (
        timeoutMs: number,
      ): Promise<MoveDiagnosticResult> => {
        let timedOut = false;
        try {
          await Promise.race([
            worldContract.simulate
              .UD__move(args, { account: diagAccount })
              .then(() => undefined),
      ...
      const handleNotSpawned = () => {
        ...
        useGameStore
          .getState()
          .setRow('Spawned', characterEntity, { spawned: false });
        onMoveRevert();
        return {
          success: false as const,
          error: 'Session expired — respawn to continue.',
        };
      };
      ...
      for (let attempt = 0; attempt < 1 + MAX_ON_CHAIN_RETRIES; attempt++) {
        try {
          ...
          if (onChainRetries > 0) {
            console.warn(
              `[move] Retry ${onChainRetries} — sending with gas ${MOVE_GAS_LIMIT} (target: ${x},${y})`,
            );
          }
          tx = await wrappedWorldContract.write.UD__move(args, {
            gas: MOVE_GAS_LIMIT,
          });
          ...
          if (receipt.status === 'reverted') {
            console.error(
              `[move] TX REVERTED — hash: ${tx}, gasUsed: ${receipt.gasUsed}, block: ${receipt.blockNumber} (target: ${x},${y})`,
            );
            ...
            if (
              ...
              console.warn(
                '[move] Position stale after revert — re-reading store',
              );
              const freshPos = (getTableValue('PositionV2', characterEntity) ??
                getTableValue('Position', characterEntity)) as
                | { x: number; y: number }
                | undefined;
              if (freshPos) {
            ...
            if (
              ...
              onChainRetries++;
              console.warn(
                `[move] Simulation now passes — retrying (${onChainRetries}/${MAX_ON_CHAIN_RETRIES})`,
              );
            ...
            if (diagnostic.result === 'error') {
              ...
              return {
                success: false,
                error: getContractError(diagnostic.error),
              };
            }
            console.warn(
              `[move] On-chain revert — ${diagnostic.result} (target: ${x},${y})`,
            );
            onMoveRevert();
            return {
              success: false,
              error: MOVE_STALE_STATE_WARNING,
              severity: 'warning',
            };
          }
          onMoveSuccess();
  ...
  const autoFight = async (
    ...
    try {
      console.info(
        `[autoFight] Sending TX — char=${characterEntity.slice(0, 10)} monster=${monsterId.slice(0, 10)} weapon=${weaponId} gas=${AUTO_FIGHT_GAS_LIMIT}`,
      );
      ...
      const receipt = await waitForTransaction(tx);
      console.info(
        `[autoFight] Receipt: status=${receipt.status} gasUsed=${receipt.gasUsed} block=${receipt.blockNumber}`,
      );

      if (receipt.status === 'reverted') {
        console.error(
          `[autoFight] ON-CHAIN REVERT — gasUsed=${receipt.gasUsed}/${AUTO_FIGHT_GAS_LIMIT} (${(Number(receipt.gasUsed) * 100) / Number(AUTO_FIGHT_GAS_LIMIT)}%)`,
        );
        try {
          ...
          console.warn(
            '[autoFight] Diagnostic simulation PASSED — was transient',
          );
        } catch (diagError) {
      ...
      console.error(
        '[autoFight] WRITE THREW (pre-send or submission error):',
        e,
      );
      if (reloadIfStaleChunk(e)) {
        return { success: false, error: 'Updating game — reloading...' };
  ...
  const restock = async (shopId: string): SystemCallReturn => {
    try {
      ...
      return {
        error:
          txResult.status === 'success' ? undefined : 'Failed to restock shop.',
        success: txResult.status === 'success',
      };
    } catch (e) {
      return {
  ...
  const sell = async (
    ...
    try {
      ...
      return {
        error:
          txResult.status === 'success' ? undefined : 'Failed to sell item.',
        success: txResult.status === 'success',
      };
    } catch (e) {
      return {
  ...
  const rest = async (entity: string): SystemCallReturn => {
    ...
    const ee = getTableValue('EncounterEntity', entity) as
      | { encounterId?: string }
      | undefined;
    if (ee?.encounterId && ee.encounterId !== '0x' + '0'.repeat(64)) {
      return {
        success: false,
        error: 'Finish the battle first.',
        severity: 'warning',
      };
    }
    ...
    try {
      ...
      const tx = await wrappedWorldContract.write.UD__rest([characterId], {
        gas: REST_GAS_LIMIT,
      });
  ...
  const useWorldConsumableItem = async (
    ...
    try {
      ...
      const tx =
        await wrappedWorldContract.write.UD__useWorldConsumableItem(args);

      const receipt = await waitForTransaction(tx);
      if (receipt.status === 'reverted') {
        try {
          await worldContract.simulate.UD__useWorldConsumableItem(args, {
            account: diagAccount,
          });
        } catch (diagError) {
          return { success: false, error: getContractError(diagError) };
      ...
      if (isInsufficientFundsError(e)) {
        try {
          await worldContract.simulate.UD__useWorldConsumableItem(
            [entity as `0x${string}`, entity as `0x${string}`, BigInt(tokenId)],
            { account: diagAccount },
          );
        } catch (diagError) {
          return { success: false, error: getContractError(diagError) };
  ...
  const rollBaseStats = async (characterEntity: string): SystemCallReturn => {
    const ownershipError = validateCharacterOwnership(
      characterEntity,
      'rollBaseStats',
    );
  ...
  const checkCombatFragmentTriggers = async (
    ...
    try {
      const tx =
        await wrappedWorldContract.write.UD__checkCombatFragmentTriggersForGroup(
          [
  ...
  const fullRespec = async (characterId: string): SystemCallReturn => {
    const ownershipError = validateCharacterOwnership(
      characterId,
      'fullRespec',
    );
  ...
  const leaveGuild = async (characterId: string): SystemCallReturn => {
    const ownershipError = validateCharacterOwnership(
      characterId,
      'leaveGuild',
    );
  ...
  const disbandGuild = async (characterId: string): SystemCallReturn => {
    const ownershipError = validateCharacterOwnership(
      characterId,
      'disbandGuild',
    );
