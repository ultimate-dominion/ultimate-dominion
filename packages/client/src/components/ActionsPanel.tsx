...
export const ActionsPanel = (): JSX.Element => {
  ...
  const { visibleOutcomes, pendingTurn, isBattleResolutionPending } =
    useCombatPacing({
      attackOutcomes,
  ...
  const { orderedItems: orderedAttackItems } = useSlotOrder(
    storageKey,
    weaponsAndSpells,
  );
  ...
  const combatConsumables = useMemo(
    () =>
      equippedConsumables.filter(
        c => c.hpRestoreAmount !== BigInt(0) && c.balance > 0n,
      ),
    [equippedConsumables],
  );
  ...
  const weaponMatchups = useMemo(() => {
    ...
    const result: Record<
      string,
      { matchup: 'strong' | 'weak' | 'neutral'; statType: StatsClasses }
    > = {};
    for (const item of orderedAttackItems) {
      ...
      if (
        (statType === StatsClasses.Strength &&
          opClass === StatsClasses.Agility) ||
        (statType === StatsClasses.Agility &&
          opClass === StatsClasses.Intelligence) ||
        (statType === StatsClasses.Intelligence &&
          opClass === StatsClasses.Strength)
      ) {
        matchup = 'strong';
      } else if (
        (statType === StatsClasses.Strength &&
  ...
  useEffect(() => {
    ...
    if (
      inlineResults.some(
        r => r.encounterId === lastestBattleOutcome.encounterId,
      )
    )
      return;
    ...
    const items = [
      ...armorTemplates
        .filter(a => ids.includes(a.tokenId))
        .map(a => ({
          ...a,
          balance: 1n,
          itemId: zeroHash,
      ...
      ...spellTemplates
        .filter(s => ids.includes(s.tokenId))
        .map(s => ({
          ...s,
          balance: 1n,
          itemId: zeroHash,
      ...
      ...weaponTemplates
        .filter(w => ids.includes(w.tokenId))
        .map(w => ({
          ...w,
          balance: 1n,
          itemId: zeroHash,
    ...
    localStorage.setItem(
      BATTLE_OUTCOME_SEEN_KEY,
      lastestBattleOutcome.encounterId,
    );
    ...
    <Box fontWeight={500} maxH="100%" overflowY="auto" ref={parentDivRef}>
      {battleOver && currentBattle && !autoAdventureMode && (
        ...
        >
                ...
                {lastestBattleOutcome.goldDropped > 0n && (
                  ...
                  >
                    <Text
                      ...
                      size="xs"
                    >
                      +{etherToFixedNumber(lastestBattleOutcome.goldDropped)}{' '}
                      Gold
                    </Text>
                  </Box>
                )}
          ...
          <Button
            ...
            _hover={{
              bg: 'rgba(212,165,74,0.12)',
              borderColor: 'rgba(212,165,74,0.6)',
            }}
          >
            View Results
          </Button>
          ...
          {canFlee && (
            <HStack justify="center" spacing={3}>
              <Button
                ...
                _hover={{
                  bg: 'rgba(184,92,58,0.20)',
                  borderColor: 'rgba(184,92,58,0.65)',
                }}
              >
                Flee
              </Button>
              <Text
                size="2xs"
                color={hasSmokeCover ? '#6B8E6B' : '#8A7E6A'}
                maxW="200px"
              >
                {hasSmokeCover
          ...
          >
            ...
            >
              {SHOW_Z2 &&
                ...
                hoveredTokenId &&
                (() => {
                  const hovItem = orderedAttackItems.find(
                    i => i.tokenId === hoveredTokenId,
                  );
                ...
                {actionItems.map((item, index) => {
                  ...
                  return (
                    <Button
                      ...
                      onMouseEnter={
                        item.type === 'attack'
                          ? () => setHoveredTokenId(item.tokenId)
                          : undefined
                      }
                      onMouseLeave={
                        item.type === 'attack'
                    ...
                    >
                      ...
                      >
                        ...
                        <HStack justify="space-between" spacing={2} w="100%">
                          <HStack spacing={1.5}>
                            {matchup === 'strong' && (
                              <Text as="span" color="#8FCB6C" fontSize="2xs">
                                ▲
                              </Text>
                            )}
                            {matchup === 'weak' && (
                              <Text as="span" color="#D89272" fontSize="2xs">
                                ▼
                              </Text>
                            )}
                            <Text
      ...
      <Stack p={{ base: 2, lg: 4 }}>
        {!currentBattle && !isSpawned && (
          ...
          >
            <Text size={{ base: 'xs', sm: 'sm', lg: 'md' }}>
              <Trans
                i18nKey="gameBoard.spawnPrompt"
                ns="ui"
                components={{ bold: <Text as="span" fontWeight={700} /> }}
              />
            </Text>
          </SafeTypist>
        )}
        {(!currentBattle || autoAdventureMode) &&
          isSpawned &&
          position &&
        ...
        {autoAdventureMode &&
          ...
          (() => {
            const hpRatio =
              Number(character.currentHp) / Number(character.maxHp);
            if (hpRatio <= 0.4) {
        ...
        {!SHOW_Z2 &&
          ...
          (() => {
            ...
            const elements = [...visibleOutcomes]
              .reverse()
              .map((attack, reverseIndex) => {
                ...
                const itemName =
                  ...
                  attack.attackerId !== character?.id
                    ? tm(`moves.${(opponent as Monster).mobId}`, {
                ...
                const alreadyAffected = attack.effectIds.some(effectId =>
                  statusEffectActions.some(
                    statusEffectAction =>
                      statusEffectAction.effectId === effectId,
                  ),
                );
                ...
                const isSelfUse =
                  attack.attackerId.toLowerCase() ===
                  attack.defenderId.toLowerCase();
                if (isSelfUse) {
                  const consumable = combatConsumables.find(
                    c => c.tokenId === attack.itemId,
                  );
                  const isPlayer =
                    attack.attackerId.toLowerCase() ===
                  ...
                  return (
                    <Box key={`battle-attack-${i}`}>
                      ...
                      >
                        <Text size={logSize}>
                          {isPlayer ? t('combat.you') : opponentDisplayName}{' '}
                          {t('combat.used')}{' '}
                ...
                const dotForTurn =
                  pendingTurn && attack.currentTurn === pendingTurn
                    ? null
                    : !seenDotTurns.has(turnKey)
                      ? dotActions.find(
                          d =>
                            d.turnNumber === attack.currentTurn &&
                            d.totalDamage > 0n,
                        )
                      : null;
                ...
                const dotMessageElement = dotForTurn ? (
                  ...
                  >
                    ...
                    >
                      ...
                      damage to{' '}
                      {dotForTurn.entityId.toLowerCase() ===
                      character?.id.toLowerCase()
                        ? 'you'
                        : opponentDisplayName}
                      .
                ...
                if (attack.miss[0]) {
                  return (
                    <Box
                      ...
                      {...(!isPlayerAttack && {
                        pl: 3,
                        borderLeft: '2px solid rgba(184,92,58,0.3)',
                      })}
                    >
                      <SafeTypist
                        avgTypingDelay={10}
                ...
                if (possibleStatusEffectAttack) {
                  ...
                  attackContent = (
                    <Box>
                      <Text size={logSize}>
                        {isCrit && (
                          <Text as="span" color="#C87A2A" fontWeight={700}>
                            {t('battle.criticalHit')}{' '}
                          </Text>
                        )}
                        {isPlayerAttack ? (
                          <Text as="span">
                      ...
                      {te(`descriptions.${possibleStatusEffectAttack.name}`, {
                        ...
                        >
                          {te(
                            `descriptions.${possibleStatusEffectAttack.name}`,
                          )}
                        </Text>
                      )}
                    </Box>
                ...
                return (
                  ...
                  >
                    ...
                    {attack.blocked && (
                      ...
                      >
                        {isPlayerAttack
                          ? t('combat.blockedSome', {
                              name: opponentDisplayName,
                            })
                          : t('combat.youBlockedSome')}
                      </Text>
                    )}
                    {attack.spellDodged && (
                      <Text
                      ...
                      >
                        {isPlayerAttack
                          ? `${opponentDisplayName} dodged the spell!`
      ...
      {selectedItem && (
        <ItemEquipModal
          isEquipped={[
            ...
            ...equippedSpells,
            ...equippedWeapons,
          ].some(i => i.name === selectedItem.name)}
          isOpen={isItemModalOpen}
          onClose={() => {
            refreshCharacter();
            onCloseItemModal();
