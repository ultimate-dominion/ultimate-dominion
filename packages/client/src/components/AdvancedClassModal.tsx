...
const ADVANCED_CLASS_INFO: Record<
  AdvancedClass,
  {
    name: string;
    description: string;
...
> = {
  ...
  [AdvancedClass.Warlock]: {
    ...
    spell:
      'Soul Drain: 8-14 magic damage + -3 STR, -3 INT on enemy for 3 turns',
...
export const AdvancedClassModal = ({
  ...
  const selectClassTx = useTransaction({
    actionName: 'select class',
    showSuccessToast: false,
  });
  const [selectedClass, setSelectedClass] = useState<AdvancedClass | null>(
    null,
  );
  const [confirmedClass, setConfirmedClass] = useState<AdvancedClass | null>(
    null,
  );
  ...
  const onConfirmClass = useCallback(async () => {
    ...
    const result = await selectClassTx.execute(async () => {
      const { error, success } = await selectAdvancedClass(
        characterId,
        selectedClass,
      );
    ...
    if (result !== undefined) {
      ...
      import('../utils/analytics').then(({ trackAdvancedClassSelected }) =>
        trackAdvancedClassSelected(
          ADVANCED_CLASS_NAMES[selectedClass] ?? 'Unknown',
        ),
      );
    }
  }, [
  ...
  const selectedInfo = selectedClass
    ? ADVANCED_CLASS_INFO[selectedClass]
    : null;
  const selectedColor = selectedClass ? CLASS_COLORS[selectedClass] : '#D4A54A';
  const confirmedInfo = confirmedClass
    ? ADVANCED_CLASS_INFO[confirmedClass]
    : null;
  const confirmedColor = confirmedClass
    ? CLASS_COLORS[confirmedClass]
    : '#D4A54A';
  ...
  return (
    <Box
      ...
      animation={
        isExiting
          ? `${exitFade} 0.6s ease-out forwards`
          : `${overlayEnter} 0.4s ease-out`
      }
    >
            ...
            {ALL_CLASSES.map((advClass, index) => {
              ...
              return (
                <Box
                  ...
                  animation={
                    showGrid
                      ? `${cardEnter} 0.6s ${0.05 * index}s cubic-bezier(0.16, 1, 0.3, 1) both`
                      : undefined
                  }
                ...
                >
                  ...
                  <Box
                    ...
                    animation={
                      isSelected
                        ? `${classGlow} 2.5s ease-in-out infinite`
                        : undefined
                    }
                    pointerEvents="none"
                  />
        ...
        >
          ...
          >
            <EntityMark
              ...
              code={confirmedInfo.name.slice(0, 3)}
              label={confirmedInfo.name}
              tone={confirmedColor}
            />
          </Box>
          ...
          <VStack
            spacing={3}
            opacity={0}
            animation={`${fadeUp} 0.6s 2.5s ease-out forwards`}
          >
            <Button
              onClick={handleClose}
              variant="gold"
