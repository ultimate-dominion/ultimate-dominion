import { encodeFunctionData, encodePacked, padHex, toHex, parseAbi, formatEther, } from 'viem';
...
const characterIdCache = new Map();
...
async function getSwapMinimum(tokenIn, tokenOut, amountIn) {
    try {
...
const CHARACTER_OWNER_TABLE_ID = encodePacked(['bytes2', 'bytes14', 'bytes16'], [toHex('tb', { size: 2 }), toHex('UD', { size: 14 }), toHex('CharacterOwner', { size: 16 })]);
// ==================== Core Functions ====================
/**
 ...
 */
export function recordFunding(eoaAddress, ethAmount) {
 ...
 */
async function getCharacterId(player) {
    ...
    try {
        ...
        const keyTuple = [padHex(player, { size: 32 })];
        ...
        if (!staticData || staticData.length < 130)
            return null; // 0x + 128 hex chars = 66 bytes min
        // Extract characterId from bytes 32-63 (hex chars 66-130)
        const characterId = ('0x' + staticData.slice(66, 130));
...
export async function flushCharges() {
    ...
    try {
        ...
        const characterIds = [];
        const counts = [];
        const countPerUnit = 1000000000000000n; // 0.001 ETH
 ...
 */
export async function swapGoldForEth() {
    try {
...
// ==================== Health ====================
function getTotalFromMap(map) {
    let total = 0n;
    for (const amount of map.values())
        total += amount;
    return total;
}
export function getPendingChargeCount() {
    return pendingCharges.size;
}
export function getTotalPendingEth() {
    return formatEther(getTotalFromMap(pendingCharges));
}
//# sourceMappingURL=gasCharge.js.map