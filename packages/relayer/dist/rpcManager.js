...
let lastCheckTime = 0;
let timer = null;
const CHECK_INTERVAL_HEALTHY = 15000; // 15s when primary is healthy
const CHECK_INTERVAL_FALLBACK = 60000; // 60s when on fallback (re-check primary)
...
async function checkPrimary() {
    try {
        const client = createPublicClient({
            chain: base,
            transport: http(config.rpcUrl, { fetchOptions, timeout: 10000 }),
        });
...
export function stopRpcHealthCheck() {
    if (timer)
        clearInterval(timer);
}
