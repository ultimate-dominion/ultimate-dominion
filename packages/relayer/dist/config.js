...
if (!singleKey && !multiKeys)
    throw new Error('Missing RELAYER_PRIVATE_KEY or RELAYER_PRIVATE_KEYS');
const relayerPrivateKeys = multiKeys
    ? multiKeys.split(',').map(k => k.trim())
    : [singleKey];
export const config = {
    relayerPrivateKeys,
    ...
    goldToken: (process.env.GOLD_TOKEN || ''),
    weth: (process.env.WETH || '0x4200000000000000000000000000000000000006'),
    swapRouter: (process.env.SWAP_ROUTER || '0x2626664c2603336E57B271c5C0b26F421741e481'),
    quoterV2: (process.env.QUOTER_V2 || '0x3d4e44Eb1374240CE5F1B871ab261CD16335B76a'),
