...
library SpellStats {
    // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "UD", name: "SpellStats", typeId: RESOURCE_TABLE });`
     ...
     */
    function get(uint256 itemId) internal view returns (SpellStatsData memory _table) {
        bytes32[] memory _keyTuple = new bytes32[](1);
        _keyTuple[0] = bytes32(uint256(itemId));
     ...
     * @notice Get the full data.
     */
    function _get(uint256 itemId) internal view returns (SpellStatsData memory _table) {
        bytes32[] memory _keyTuple = new bytes32[](1);
        _keyTuple[0] = bytes32(uint256(itemId));
    ...
    function decode(bytes memory _staticData, EncodedLengths _encodedLengths, bytes memory _dynamicData)
        internal
        pure
        returns (SpellStatsData memory _table)
    {
