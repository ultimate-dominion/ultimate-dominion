...
library Spawned {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "UD", name: "Spawned", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x74625544000000000000000000000000537061776e6564000000000000000000);
   ...
   */
  function encodeStatic(bool spawned) internal pure returns (bytes memory) {
   ...
   */
  function encode(bool spawned) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(spawned);
