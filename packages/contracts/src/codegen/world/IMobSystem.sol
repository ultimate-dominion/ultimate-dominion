...
interface IMobSystem {
    function UD__createMob(MobType mobType, bytes memory stats, string memory mobMetadataUri)
        external
        returns (uint256);

    function UD__createMobs(MobType[] memory mobTypes, bytes[] memory stats, string[] memory mobMetadataURIs)
        external;
